...
#         # new row detailing the new Organization in the public schema with its associated schema name
...
def provision_organization(name: str, subdomain: str, schema_name: str):
    with SessionLocal() as session:
        ...
        # 2. Record the Organization in Public
        new_org = app.models.Organization(name=name, subdomain=subdomain, schema_name=schema_name)
        session.add(new_org)
        session.commit()
        

    # 3. Run Alembic Migrations for this specific schema
    alembic_cfg = Config("alembic.ini")
    
    ''' These match the fallback keys we added to env.py so env.py can read 
        them whether we run this script or use CLI args.
        We pass the schema name into the connection string or as a variable
        so env.py knows which schema to target '''
    alembic_cfg.attributes['is_tenant'] = True
    alembic_cfg.attributes['target_schema'] = schema_name
    
    # This triggers the run_migrations_online() function in env.py
